...
while ( $i < $start_config->{total_months} ) {
  ...
  $month = get_assets_value($month) and print "month $i:\n" . $month if $old_num_aps != $month->{num_aps};
  print "apartment no:" . $month->{num_aps} . " paid in month no: $i\n" if $prev_debt > 0 && $month->{debt} == 0;
...
print "Last month stat " ; print Dumper($start_config);
